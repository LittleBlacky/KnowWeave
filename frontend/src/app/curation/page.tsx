  ...
  return (
    <AppShell>
      ...
      <div className="mb-6 grid grid-cols-4 gap-4 max-2xl:grid-cols-2 max-sm:grid-cols-1">
        <StatCard
          label="知识分块"
          value={report.total_chunks}
          icon={BarChart3}
        />
        <StatCard
          label="知识单元"
          value={report.total_knowledge_units}
          icon={BookOpenCheck}
        />
        <StatCard
          label="Wiki 页面"
          value={report.total_wiki_pages}
          icon={BookOpenCheck}
        />
        <StatCard
          label="反馈信号"
          value={report.total_feedback_count}
          icon={MessageSquare}
        />
      </div>
      ...
      <section className="mb-6 rounded-md border border-[#dcded8] bg-[#e1ebe7] p-4">
        ...
        <p className="text-sm whitespace-pre-wrap text-[#123d37]">
          {report.summary}
        </p>
      </section>
        ...
        <section className="rounded-md border border-[#dcded8] bg-white">
          ...
          <div className="divide-y divide-[#dcded8]">
            {report.high_value_chunks.length === 0 ? (
              <p className="px-4 py-6 text-sm text-[#6f756f]">
                暂无高价值分块。
              </p>
            ) : (
              report.high_value_chunks.map((chunk) => (
                <div key={chunk.chunk_id} className="px-4 py-3">
                  ...
                  <p className="mt-1 text-xs text-[#6f756f] line-clamp-2">
                    {chunk.preview}
                  </p>
