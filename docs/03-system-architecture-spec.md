...
- P0 支持 fixed_size、paragraph、heading、hybrid 策略；P1 支持 parent_child 策略。
- 维护 source spans。
- MVP 预留 `parent_chunk_id` 并可展示调试信息；完整父子 chunk 关系生成和上下文扩展放入 P1。
- 计算 chunk 质量信号。
...
5. `docs/08-frontend-spec.md`
   - 定义页面信息架构、chunk 治理 UI、Source Viewer、Wiki/Search/Chat 交互和流式 Markdown 渲染。

6. `docs/09-acceptance-test-spec.md`
   - 定义 P0/P1/P2 验收测试、演示剧本、失败分级和验收报告模板。

7. `docs/10-evaluation-spec.md`
   - 定义 evaluation datasets、evaluation_runs、准确率、召回率、引用命中率和回归评估。

8. `docs/11-backend-implementation-spec.md`
   - 定义 FastAPI 目录结构、SQLAlchemy models、迁移、Service 层、Provider 抽象和测试策略。

9. `docs/12-frontend-implementation-spec.md`
   - 定义 Next.js 页面路由、组件结构、API client、状态管理和前端测试策略。

10. `docs/13-devops-and-demo-spec.md`
   - 定义 Docker Compose、PostgreSQL + pgvector 初始化、环境变量、演示数据和启动脚本。

下一步进入工程骨架实现，优先创建 `backend/`、`frontend/`、`docker-compose.yml`、`.env.example` 和 P0 smoke。
