...
| 状态 | 含义 | 用户可见动作 |
| --- | --- | --- |
| `rejected` | 上传校验失败，文件未进入可用知识库 | 查看失败原因、重新上传 |
...
- 创建 `rejected` 状态记录，或在存储失败前直接返回错误。
- `rejected` 文件不得进入解析、检索、问答和 Wiki 沉淀。
- 返回明确错误原因。
...
检测到但无法解析的内容不得直接丢弃，应保存为 `unknown` 或对应 placeholder。`table`、`image`、`formula` 在 P0 可以只保存占位内容和位置，深度结构化解析放到 P2。
...
| `is_ignored` | boolean | 是否被用户排除在 chunking 之外；落库字段为 `document_blocks.is_ignored` |
...
- `document_blocks.is_ignored = true` 的 block 不应进入 chunk。
...
- 若用户保存过编辑内容，chunk 应写入 `is_manually_edited = true`。
...
- `parse_results.status` 仍使用 `parse_succeeded` 或 `parse_failed` 表示一次 parser 运行是否完成。
- `parse_needs_review` 是 `knowledge_files.status`，表示文件可用但需要人工检查。
- 具体 warning 详情写入 `parse_results.warnings`。
