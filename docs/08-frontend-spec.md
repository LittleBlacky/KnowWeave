...
- 完整测试用例矩阵。后续放入 `09-acceptance-test-spec.md`。
- 评测运行和指标计算。后续放入 `10-evaluation-spec.md`。
...
| Evaluation | `/evaluation` | P1 | 样本、运行和指标 |
...
| curation_status | 是 | draft、verified、ignored、archived |
...
- 保存后 curation_status 可保持 draft，也可由用户显式设为 verified。
...
- 列表中用标签展示 flags。
- 详情页展示具体原因和关联证据。
- 用户可以确认“接受该 chunk”，系统保留 flags 但 curation_status 变为 verified。
