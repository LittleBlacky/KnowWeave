...
1. 文本上传：将原始文件纳入知识库。
2. 文本解析：从文件中提取可处理文本和结构信息。
3. 文本分块：将长文本切成可检索、可引用、可治理的 chunk。
...
MVP 不需要实现完整音视频处理，但需要在数据模型中允许 source_position 表达时间戳，例如 `start_time=00:03:20,end_time=00:05:10`，并允许 chunk 类型为 transcript 或 mixed。

### 3.4 Document Block 通用结构
...
- chunk 的核心检索字段可以仍然是 content 和 summary。
- metadata 必须保留原始结构信息，避免后续无法重建表格、图片或公式语义。
- source_position 必须能定位到文件页码、段落、表格区域、图片编号或公式编号。
- status 应对所有类型生效，例如 verified、ignored、archived。
...
- 对音视频文件预留 Timeline Block 生成能力，MVP 可只记录媒体元数据和待处理状态。
- 保留 block 阅读顺序。
- 保留嵌入内容与前后文本的上下文关系。
- 记录页码或段落位置。
- 保存解析结果版本。
...
- 对音视频文件，系统至少能保留媒体时长、待转写状态和时间轴占位能力。

## 6. 阶段三：文本分块

### 6.1 阶段目标

将长文本拆分成适合检索、问答引用和人工治理的 chunk。分块质量直接影响后续检索和问答质量，因此必须允许用户调参和人工修正。
...
- 从搜索结果生成主题知识单元。
- 从问答结果保存知识单元。
- 查看一次问答命中的召回 chunk 和引用来源。
- 将用户问题、AI 回答、召回 chunk、引用来源和用户反馈保存为对话样本。
- 将高质量对话样本加入评测集。
- 将负反馈对话加入待优化队列。
- 手动创建知识单元。
...
- question：用户问题。
- answer：AI 回答。
- retrieved_chunks：本次召回的 chunk 列表。
